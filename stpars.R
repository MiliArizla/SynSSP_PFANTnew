...
print('logg_cn = stars with logg <= logg_cn --> RGBs with different [C/Fe] and [N/Fe] ratios, default logg_cn = 3.0')
...
# FUNCTION STPARS
####################################################################
stpars <- function(n_ms, n_rg, feh, afe, age, logg_cn, fig){
  
  if(missing(fig)){fig = F}
  if(missing(logg_cn)){logg_cn = 3}
  
  #---------------------------------
  # SET OUTPUT FILENAME
  #---------------------------------
  output.file <- set.stpars.filename(n_ms, n_rg, feh, afe, age, logg_cn)
  ...
  for(i in 1: length(teff_grid)){
    ...
    if(logg_grid[i] <= logg_cn){phase[i] = 'rgb_cn'}
  }
  ...
  # WRITE OUTPUT FILE
  #---------------------------------
  if(!file.exists('Stellar_pars')){system('mkdir Stellar_pars')}
  
  data <- sprintf('%10s%10s%12s%10s%10s', '# Teff/K', 'logg', 'Mass/Msun',  'logL/Lsun', 'phase')
  write(data, file = paste(output.file, sep =''), ncolumns = 100)
  for(i in 1:(n_ms + n_rg + 1)){
    data <- sprintf('%10.0f%10.2f%12.7f%10.4f%10s', 
                    iso.teff.grid[i], iso.logg.grid[i], iso.mass.grid[i], iso.logL.grid[i], phase[i])
  ...
  if(missing(fig) == F & fig != F){
    ...
    points(iso.teff.grid[phase == 'rgb_cn'], iso.logg.grid[phase == 'rgb_cn'], col = 'black', pch = 21, cex = 1.6, lwd = 1.5)
...
##################################
set.stpars.filename <- function(n_ms, n_rg, feh, afe, age, logg_cn){
  if(missing(logg_cn)){logg_cn = 3}
  ...
  logg_cn_s = sprintf('%1.1f', logg_cn)
  file = paste('./Stellar_pars/stpars_', 'fe', feh_s, 'a', afe_s, 'age', age_s, 
                      'ms', n_ms_s, 'rg', n_rg_s, 'loggCN', logg_cn_s, '.dat', sep = '')
