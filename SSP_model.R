...
# CHANGE LOG
# ****************************************
# 24/07/18: Change in nullbad call for new nullbad
# Tatiana 
...
# FUNCTION SSP.MODEL
####################################################################
ssp.model <- function(feh, afe, age, imf, slope, fwhm, dl, CFe, CFe_rgb, NFe, NFe_rgb,
                      OFe, MgFe, SiFe, CaFe, TiFe, NaFe, AlFe, BaFe, EuFe, n_ms, n_rg, logg_cn, parfile, lmin, lmax){
  ...
  # OUTPUT FILE NAMES
  #---------------------------------
  if(!file.exists('SSP_Spectra')){system('mkdir SSP_Spectra')}
  
  file_ssp <- set.ssp.filename(feh, afe, CFe, CFe_rgb, NFe, NFe_rgb, OFe, MgFe, SiFe, CaFe, TiFe, NaFe, AlFe, BaFe, EuFe, age, imf, slope)
  ...
  for(i in 1:nstars){
    ...
    nulbad_call = sprintf("nulbad --fn_flux temp.in --fn_cv st_spectra --flam T --pat %f --fn_progress progress.txt --fwhm %f", dl, fwhm)
...
##################################
set.ssp.filename <- function(feh, afe, CFe, CFe_rgb, NFe, NFe_rgb, OFe, MgFe, SiFe, CaFe, TiFe, NaFe, AlFe, BaFe, EuFe, age, imf, slope){
  #---------------------------------
  # SETTING DEFAULT VALUES
  #---------------------------------
  if(missing(CFe)){CFe = 0.0}
  ...
  if(CFe_rgb == CFe & NFe_rgb == NFe){
    CN_rgb = ''
  }else{
    if(CFe_rgb >= 0){tempC = sprintf('%1s%4.2f', '+', CFe_rgb)}else{tempC = sprintf('%1s%4.2f', '-', CFe_rgb)}
    if(NFe_rgb >= 0){tempN = sprintf('%1s%4.2f', '+', NFe_rgb)}else{tempN = sprintf('%1s%4.2f', '-', NFe_rgb)}
    
    CN_rgb = sprintf('CNrgb%s_%s', CFe_rgb, NFe_rgb)
  }
  ...
  if(imf == 'unimodal'){
    ...
    file_ssp = paste('./SSP_Spectra/SSP_Fe', temp_s[1],  '_a', temp_s[2], '_C', temp_s[3], '_N', temp_s[4],
                     ...
                     '_Na', temp_s[10], '_Al', temp_s[11], '_Ba', temp_s[12], '_Eu', temp_s[13], CN_rgb,
                     '_age', age_s, '_slope', slope_s, sep = '')
  }else{
    file_ssp = paste('./SSP_Spectra/SSP_Fe', temp_s[1],  '_a', temp_s[2], '_C', temp_s[3], '_N', temp_s[4],
                      '_O', temp_s[5],'_Mg', temp_s[6], '_Si', temp_s[7],'_Ca', temp_s[8],'_Ti', temp_s[9], 
                     '_Na', temp_s[10], '_Al', temp_s[11], '_Ba', temp_s[12], '_Eu', temp_s[13], CN_rgb,
                     '_age', age_s, '_', imf, sep = '')
  }
}
